...
export function add_product_image(req, res) {
  ...
  if (req.headers.admin_token != "") {
    req.vendor_id = base64_images[0]["vendor_id"];
  }
      ...
      connection.query(
        ...
        (err, rows, fields) => {
          ...
          if (index === iterations) {
            ...
            console.log(
              "product_image_data------------check-----------46-------test===="
            );
            console.log(product_image_data);
            res.status(200).json({
              status: true,
              response: "successfully add images",
              product_image_data,
            });
          }
        }
...
export function product_image_update(req, res) {
  ...
  base64_images.forEach(function (item, index) {
    ...
    try {
      ...
      var base64Data = imgBase64.replace("data:image/png;base64,", "");
      var name_str =
        "" + item.product_image_name.split(" ").join("") + "_" + img_num + "";
      ...
      query_ =
        'UPDATE `product_images` SET `product_image_name`="' +
        name_str +
...
export function add_remove_cover_image(req, res) {
  var { image_position, product_image_id, product_id, product_verient_id } =
    req.body;
...
export function product_image_delete(req, res) {
  var { product_image_id, product_id, product_image_name, product_verient_id } =
    req.body;
    ...
    (err, rows, fields) => {
      if (err) {
        ...
        if (rows.affectedRows == "1") {
          ...
          res.status(200).send({
            response: "product_id or product_image_id not match",
            status: false,
          });
        }
      }
