...
export function trending_products(req, res) {
  ...
  if (req.user_id != "" && req.user_id != undefined) {
    query_ =
      ...
      req.user_id +
      '") AS wishlist  FROM `order` LEFT JOIN `product_view` ON `order`.product_id = `product_view`.product_id WHERE verient_is_deleted = 0 AND   ';

    query_for_count_row =
      "SELECT COUNT(DISTINCT(`order`.product_id)) as total_count FROM `order` LEFT JOIN `product_view` ON `order`.product_id = `product_view`.product_id WHERE verient_is_deleted = 0 AND   ";
  } else {
    query_ =
      "SELECT COUNT(*) as total_count, COUNT(`order`.product_id) AS sale_count, `order`.product_id, `order`.created_on,`product_view`.`id`,`product_view`.`vendor_id`,`product_view`.`name`,`product_view`.`category_name`,`product_view`.`seo_tag`,`product_view`.`brand`,`product_view`.`category`,`product_view`.`is_deleted`,`product_view`.`status`,`product_view`.`review`,`product_view`.`rating`,`product_view`.`description`,`product_view`.`is_active`,`product_view`.`created_by`,`product_view`.`created_by_id`,`product_view`.`created_on`,`product_view`.`updated_on`,`product_view`.`product_verient_id`,`product_view`.`product_id`,`product_view`.`verient_name`,`product_view`.`quantity`,`product_view`.`unit`,`product_view`.`product_stock_quantity`,`product_view`.`price`,`product_view`.`mrp`,`product_view`.`gst`,`product_view`.`sgst`,`product_view`.`cgst`,`product_view`.`verient_is_deleted`,`product_view`.`verient_status`,`product_view`.`discount`,`product_view`.`verient_description`,`product_view`.`verient_is_active`,`product_view`.`verient_created_on`,`product_view`.`verient_updated_on`,`product_view`.`product_height`,`product_view`.`product_width`,`product_view`.`product_Weight`,`product_view`.`cover_image`,`product_view`.`all_images_url`,`product_view`.`avgRatings` FROM `order` LEFT JOIN `product_view` ON `order`.product_id = `product_view`.product_id WHERE verient_is_deleted = 0 AND   ";
  ...
  if (from_date && to_date) {
    ...
    query_ +=
      "(`order`.created_on BETWEEN '" + from_date + "' AND '" + to_date + "')";
    query_for_count_row +=
      "`order`.created_on BETWEEN '" + from_date + "' AND '" + to_date + "'";
  }
  ...
  if (from_price && to_price) {
    query_ +=
      ' AND (`product_view`.price BETWEEN "' +
      from_price +
      '" AND "' +
      to_price +
      '")';
  ...
  connection.query(query, (err, results) => {
    ...
    console.log(
      "--------query_for_count_row-------------" + query_for_count_row
    );
...
export function add_fetured_product(req, res) {
  ...
  connection.query(
    'SELECT * FROM `fetured_product_table` WHERE `product_id`="' +
      product_id +
      '" AND `fetured_type`="' +
      fetured_type +
      '" AND is_deleted="1"',
    (err, rows, fields) => {
      if (err) {
        ...
        if (rows.length) {
          ...
          connection.query(
            'INSERT INTO `fetured_product_table`(`product_id`, `fetured_type`, `start_date`, `end_date`) VALUES ("' +
              product_id +
              '","' +
              fetured_type +
              '","' +
            ...
            (err, rows, fields) => {
              if (err) {
                ...
                rows != ""
                  ? res.status(200).send({ status: true, results: rows })
...
export function update_fetured_product(req, res) {
  ...
  for (let k in req_body) {
    if (
      ["start_date", "is_deleted", "end_date", "status"].includes(k) &&
      req_body[k]
    ) {
      qry_ += ` , ${k} = '${req_body[k]}' `;
    }
  }
  console.log(qry_ + " WHERE id = '" + req.body.id + "'");
  connection.query(
    qry_ + " WHERE id = '" + req.body.id + "'",
    (err, rows, fields) => {
      if (err) {
        console.log(err);
