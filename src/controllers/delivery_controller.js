...
export function sign_by_driver(req, res) {
  ...
  if (req.body.email != "" && req.body.password != "") {
    ...
    if (regex.test(u_email)) {
      connection.query(
        "SELECT * FROM delivery_man WHERE email = '" + u_email + "'",
        (err, rows) => {
          if (err) {
        ...
        if (err) {
          ...
          if (rows != "") {
            if (user_otp == rows[0].otp) {
              connection.query(
                ...
                (err, rows) => {
                  if (err) {
                    ...
                    if (err.code == "ER_DUP_ENTRY") {
                      connection.query(
                        "SELECT * FROM delivery_man WHERE email = '" +
                          user_email +
                          "' ",
                        (err, rows) => {
                          if (err) {
                            ...
                            if (rows != "") {
                              ...
                              jwt.sign(
                                { id: rows[0].driver_id },
                                process.env.DRIVER_JWT_SECRET_KEY,
                                function (err, token) {
                                  res.status(200).json({
                                    success: true,
                                    token: token,
              ...
              res
                .status(200)
                .send({ response: "otp not matched", status: false });
            }
          } else {
            res
              .status(200)
              .send({ response: "email not matched", status: false });
          }
        }
      }
    );
...
export function driver_login(req, res) {
  ...
  if (req.body.email != "" && req.body.password != "") {
    if (regex.test(user_email)) {
      console.log("true");
      connection.query(
        'SELECT * FROM delivery_man WHERE  email ="' + user_email + '"',
        (err, rows) => {
          if (err) {
            console.log(err);
...
export function driver_forgate_password_update(req, res) {
  console.log(
    "________________delivery_man_forgate_password_update--------------------" +
      req.driver_token
  );
  let psw = req.body.password.trim();
  console.log(psw);
  console.log(
    "update delivery_man  set `password`= '" +
      psw +
      "' where driver_id ='" +
      req.driver_id +
      "'"
...
export function set_driver_notification_token(req, res) {
  ...
  connection.query(
    "update delivery_man  set `fcm_token`= '" +
      not_token +
      "' where driver_id ='" +
      req.driver_id +
      "'",
...
export async function driver_details(req, res) {
  ...
  // return false
  connection.query(
    "SELECT * FROM delivery_man LEFT JOIN vehicle_detaile ON delivery_man.driver_id = vehicle_detaile.driver_id where delivery_man.driver_id= '" +
      req.driver_id +
      "'",
    (err, rows) => {
      if (err) {
...
export async function only_driver_list(req, res) {
  ...
  connection.query(
    query_ + "ORDER BY delivery_man.created_on DESC",
    (err, rows) => {
      if (err) {
        console.log(err);
        res
...
export async function delete_restore_driver(req, res) {
  ...
  if (driver_id != "" && is_active != "" && status != "") {
    ...
    if (is_active != "") {
      query_ +=
        " `is_active`='" + is_active + "' where driver_id ='" + driver_id + "'";
    }
    if (status != "") {
      query_ +=
...
export async function update_driver(req, res) {
  ...
  let srt_user =
    "update delivery_man  set `driver_name`='" +
    driver_name +
    "', `driver_last_name`='" +
    driver_last_name +
    "', `date_of_birth`='" +
  ...
  for (let k in req.files) {
    ...
    }/driver_profile/${req.files[k][0]["filename"]}'`;
  }

  if (req.headers.admin_token != "" && req.headers.admin_token != undefined) {
    srt_user += " where driver_id ='" + driver_id + "'";
  } else if (
    req.headers.driver_token != "" &&
    req.headers.driver_token != undefined
  ) {
...
export function add_working_area(req, res) {
  let { city, area_name, pin_code, driver_log, driver_lat, driver_id } =
    req.body;
  let query_ = "";
  if (req.headers.admin_token != "" && req.headers.admin_token != undefined) {
    if (!driver_lat && !driver_log) {
      driver_lat = 0;
      driver_log = 0;
    }
    query_ +=
      "INSERT INTO `driver_working_area`(`driver_id`,`city`, `area_name`, `pin_code`, `driver_log`, `driver_lat`) VALUES (" +
      driver_id +
      ",'" +
      city +
      "','" +
  ...
  connection.query(query_, (err, rows) => {
    if (err) {
      ...
      res.status(StatusCodes.OK).json({
        message: "added yuor working area successfully",
        status: true,
      });
    }
  });
...
export function chouse_driver_for_delivery(req, res) {
  let { order_id, delivery_lat, delivery_log, nearest_of_delivery_pin } =
    req.body;
  let query_ = "";
  if (
    nearest_of_delivery_pin != "" &&
    nearest_of_delivery_pin != undefined &&
    nearest_of_delivery_pin != null
  ) {
    query_ +=
      "SELECT *, ( 3959 * acos( cos( radians(" +
      delivery_lat +
      ") ) * cos( radians( driver_lat ) ) * cos( radians( driver_log ) - radians(" +
      delivery_log +
      ") ) + sin( radians(" +
      delivery_lat +
      ") ) * sin( radians( driver_lat ) ) ) ) AS distance FROM driver_working_area  LEFT JOIN delivery_man ON driver_working_area.driver_id = delivery_man.driver_id AND vehicle_detaile.is_active = '1' AND delivery_man.is_active = '1' HAVING distance < " +
      nearest_of_delivery_pin +
      " ";
  } else {
    ...
    query_ +=
      "SELECT delivery_man.*,vehicle_detaile.model, vehicle_detaile.is_active AS vehicle_is_active FROM delivery_man,vehicle_detaile where delivery_man.driver_id = vehicle_detaile.driver_id AND vehicle_detaile.is_active = '1' AND delivery_man.is_active = '1' GROUP BY delivery_man.driver_id;";
  }
  console.log(query_);
  connection.query(query_, (err, rows) => {
    if (err) {
...
export function register_your_vehicle(req, res) {
  ...
  if (req.headers.admin_token != "" && req.headers.admin_token != undefined) {
    srt_user =
      "INSERT INTO `vehicle_detaile`(`vehicle_add_by`,`company_name`, `model`, `color`, `registration_no_of_vehicle`, `chassis_number`, `vehicle_owner_name`, `puc_expiration_date`, `insurance_expiration_date`, `registration_expiration_date`" +
      str_fields +
      ") VALUES('admin','" +
      company_name +
      "', '" +
    ...
    connection.query(
      "UPDATE `vehicle_detaile` SET `is_active` = '0' WHERE `vehicle_detaile`.`driver_id` = '" +
        req.driver_id +
        "'",
      (err, rows) => {}
    );
    ...
    srt_user =
      "INSERT INTO `vehicle_detaile`(`vehicle_add_by`,`driver_id`, `company_name`, `model`, `color`, `registration_no_of_vehicle`, `chassis_number`, `vehicle_owner_name`, `puc_expiration_date`, `insurance_expiration_date`, `registration_expiration_date`" +
      str_fields +
      ") VALUES( 'driver','" +
      req.driver_id +
      "', '" +
  ...
  connection.query(srt_user, (err, rows) => {
    if (err) {
      ...
      if (err["errno"] == "1062") {
        res
          .status(StatusCodes.INTERNAL_SERVER_ERROR)
          .json({ message: err["sqlMessage"], status: false });
      } else {
        res
          .status(StatusCodes.INTERNAL_SERVER_ERROR)
          .json({ message: "something went wrong", status: false });
      ...
      res
        .status(StatusCodes.OK)
        .json({ message: "vehicle registration successfull", status: true });
    }
  });
}

export function order_asign(req, res) {
  let { order_id, payment, payment_method, order_delivery_confirm_code } =
    req.body;
  console.log({
    order_id,
    payment,
  ...
  connection.query(
    ...
    (err, rows) => {
      if (err) {
        ...
        if (rows != "") {
          connection.query(
            "INSERT INTO `order_delivery_details`(`order_id`, `payment`,  `payment_method`, `order_delivery_confirm_code`,`order_ready_to_asign_for_delivery_by`) VALUES ('" +
              order_id +
              "','" +
              payment +
              "', '" +
          ...
          res
            .status(StatusCodes.OK)
            .json({ status: false, response: "order not verify by vendor" });
        }
      }
    }
  );
...
export function order_asign_by_delivery_admin(req, res) {
  ...
  connection.query(
    "SELECT * FROM `vehicle_detaile` WHERE driver_id = " +
      driver_id +
      " AND is_active='1'",
    (err, rows) => {
      if (err) {
        ...
        if (rows != "") {
          ...
          console.log(
            "UPDATE `order_delivery_details` SET `driver_id`='" +
              driver_id +
              "', `vehicle_id`='" +
              rows[0]["vehicle_id"] +
              "',`order_asign_by`='" +
              req.created_by_id +
              "',`last_modification_by`='delivery_admin',`last_modification_by_id`='" +
              req.created_by_id +
              "', `order_asign_date` = NOW() WHERE order_id = " +
              order_id +
              ""
          );
          connection.query(
          ...
          res
            .status(StatusCodes.OK)
            .json({ message: "driver has no vehicle", status: false });
        }
      }
    }
  );
...
export function get_delivery_detaile_list(req, res) {
  ...
  if (req.headers.admin_token) {
    if (
      ...
      filter +=
        " order_delivery_details.created_on between '" +
        req.body.date_from +
        " 00:00:00' AND '" +
        req.body.date_to +
        " 23:59:59' AND  ";
  ...
  if (req.headers.driver_token) {
    if (
      ...
      filter +=
        " order_delivery_details.order_asign_date between '" +
        req.body.date_from +
        " 00:00:00' AND '" +
        req.body.date_to +
        " 23:59:59' AND  ";
  ...
  connection.query(
    filter +
      "GROUP BY order_delivery_details.order_id ORDER BY order_delivery_details.created_on DESC",
    (err, rows) => {
      if (err) {
        console.log(err);
        res
...
export function delivery_area_list(req, res) {
  //old_code
  let qyery_ =
    "SELECT *,(SELECT driver_name FROM `delivery_man` WHERE delivery_man.driver_id = driver_working_area.driver_id ) AS driver_name FROM `driver_working_area` WHERE";
...
export function active_deactive_area(req, res) {
  ...
  connection.query(query_ + " WHERE id=" + id + "", (err, rows) => {
    if (err) {
      ...
      if (rows.affectedRows >= 1) {
        res.status(StatusCodes.OK).json({
          message: "successfull changed working area status  ",
          status: true,
        });
      } else {
        res
...
export function change_order_detaile_status(req, res) {
  ...
  if (
    ...
    req.headers.admin_token != undefined &&
    req.headers.admin_token != null
  ) {
    query_ =
      "UPDATE `order_delivery_details` SET `order_status`='" +
      order_status +
      "', `last_modification_by`='admin', `last_modification_by_id`='" +
      req.admin_id +
      "'";
  ...
  if (
    req.headers.driver_token != "" &&
    req.headers.driver_token != undefined &&
    req.headers.driver_token != null
  ) {
    query_ =
      "UPDATE `order_delivery_details` SET `order_status`='" +
      req.body.order_status +
      "', `last_modification_by`='delivery_man', `last_modification_by_id`='" +
      req.driver_id +
      "',`status_comment`='" +
      req.body.status_comment +
      "'";
    query_ +=
      " WHERE order_id ='" +
      order_id +
  ...
  connection.query(query_, (err, rows) => {
    if (err) {
      ...
      if (rows.affectedRows >= 1) {
        ...
        res
          .status(StatusCodes.OK)
          .json({ message: "status changed successfull", status: true });
      } else {
        res
          .status(StatusCodes.INTERNAL_SERVER_ERROR)
          .json({ message: "something went wrong", status: false });
...
export function delivery_verify_code_match(req, res) {
  ...
  connection.query(
    "SELECT * FROM `order_delivery_details` WHERE driver_id='" +
      req.driver_id +
      "' AND order_id='" +
      order_id +
      "' AND order_delivery_confirm_code ='" +
      order_delivery_confirm_code +
      "' ",
    (err, rows) => {
      if (err) {
        ...
        if (rows != "") {
          connection.query(
            "UPDATE `order_delivery_details` SET `order_status`='delivered',`delivered_date` = NOW() WHERE driver_id='" +
              req.driver_id +
              "' AND order_id='" +
              order_id +
              "' AND order_delivery_confirm_code ='" +
...
console.log(rows)
          ...
          connection.query(
            "UPDATE `order` SET `status_order`='delivered' WHERE order_id ='" +
              order_id +
              "'",
            (err, rows) => {
              console.log("---" + err);
          ...
          res
            .status(200)
            .json({ message: "not match credintials", status: false });
        }
      }
    }
  );
...
export function driver_add_by_admin(req, res) {
  ...
  console.log(
    "INSERT INTO `delivery_man`(`driver_name`, `driver_last_name`, `date_of_birth`, `current_address`, `gender`, `age`, `contect_no`, `email`, `password`,`aadhar_no`, `licence_no`, `licence_issue_date`, `licence_validity_date`" +
      str_fields +
      ") VALUES ( '" +
      driver_name +
      "', '" +
    ...
    (err, rows) => {
      if (err) {
        ...
        if (err.code == "ER_DUP_ENTRY") {
          res
            .status(200)
            .json({ message: "account already exists", status: false });
        } else {
          res
            .status(StatusCodes.INTERNAL_SERVER_ERROR)
            .json({ message: "something went wrong", status: false });
...
export async function order_details_for_driver(req, res) {
  ...
  if (req.headers.driver_token) {
    qry_ =
      'select * from `order` ,`order_delivery_details` where order_delivery_details.order_id = `order`.order_id AND order_delivery_details.order_id =  "' +
      id +
      '" AND order_delivery_details.driver_id = "' +
      req.driver_id +
      '" ';
  }
  ...
  connection.query(qry_, (err, rows) => {
    if (err) {
      console.log(err);
...
export function vehicle_list(req, res) {
  ...
  if (req.headers.driver_token) {
    query_ +=
      "SELECT *,(SELECT driver_name FROM `delivery_man` WHERE delivery_man.driver_id = vehicle_detaile.driver_id) AS driver_name FROM `vehicle_detaile` WHERE driver_id = '" +
      req.driver_id +
      "' AND  ";
  } else {
    if (req.headers.admin_token) {
      query_ +=
        "SELECT *,(SELECT driver_name FROM `delivery_man` WHERE delivery_man.driver_id = vehicle_detaile.driver_id) AS driver_name FROM `vehicle_detaile` WHERE";
    }
  }
...
export function update_your_vehicle(req, res) {
  ...
  if (req.headers.admin_token != "" && req.headers.admin_token != undefined) {
    srt_user =
      "UPDATE `vehicle_detaile` SET `company_name`='" +
      company_name +
      "', `model`='" +
      model +
      "', `color`='" +
  ...
  connection.query(srt_user, (err, rows) => {
    if (err) {
      ...
      if (rows.affectedRows == "1") {
        res
          .status(StatusCodes.OK)
          .json({ message: "vehicle update successfull", status: true });
      } else {
        res
          .status(StatusCodes.INTERNAL_SERVER_ERROR)
          .json({ message: "something went wrong", status: false });
...
export function change_vehicle_feild(req, res) {
  ...
  if (status) {
    if (status == "deleted" || status == "block" || status == "pendding") {
      ...
      connection.query(
        "UPDATE `vehicle_detaile` SET `status` = '', `is_active` = '0' WHERE `vehicle_detaile`.`vehicle_id` = '" +
          vehicle_id +
          "'",
        (err, rows) => {
          if (err) {
            ...
            res.status(200).json({
              message: "vehicle feild updated successfull",
              status: true,
            });
          }
        }
      ...
      connection.query(
        "UPDATE `vehicle_detaile` SET `status` = '', `is_active` = '1' WHERE `vehicle_detaile`.`vehicle_id` = '" +
          vehicle_id +
          "'",
        (err, rows) => {
          if (err) {
      ...
      res
        .status(200)
        .json({ message: "please send valid status", status: false });
    }
  } else {
    console.log("UPDATE `vehicle_detaile` SET");
    console.log(req.body);
    if (req.headers.driver_token && req.driver_id) {
      connection.query(
        "UPDATE `vehicle_detaile` SET `driver_id` = '', `is_active` = '0' WHERE `vehicle_detaile`.`driver_id` = '" +
          req.driver_id +
          "' AND 	vehicle_add_by = 'admin'",
        (err, rows) => {}
      );
      connection.query(
        "UPDATE `vehicle_detaile` SET `is_active` = '0' WHERE `vehicle_detaile`.`driver_id` = '" +
          req.driver_id +
          "' AND 	vehicle_add_by = 'driver'",
        (err, rows) => {}
      );
      req.body.driver_id = req.driver_id;
    }
