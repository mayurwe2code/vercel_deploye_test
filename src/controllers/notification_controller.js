...
export function notification(req, res) {
  ...
  connection.query(
    'SELECT * FROM `notification` WHERE (actor_id = "' +
      req.user_id +
      '" OR actor_id = 0 ) AND actor_type = "user" ORDER BY created_on DESC',
    (err, rows, fields) => {
      if (err) {
        ...
        if (rows != "") {
          res.status(200).send(rows);
          connection.query(
            'UPDATE `notification` SET `status`="read" WHERE actor_id = "' +
              req.user_id +
              '" AND actor_type = "user"',
            (err, rows, fields) => {
              if (err) {
...
export function admin_notification(req, res) {
  ...
  connection.query(
    str_notification + " ORDER BY id DESC",
    (err, rows, fields) => {
      if (err) {
        //console.log("/notification" + err)
        res.status(200).send(err);
      ...
      (err, rows) => {
        if (err) {
          ...
          if (rows.affectedRows == "1") {
            res
              .status(200)
              .send({ response: " delete notification successfully" });
          } else {
            res.status(200).send({ response: "delete opration failed" });
          }
        }
