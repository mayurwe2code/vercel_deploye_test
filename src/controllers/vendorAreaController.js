...
export function vendor_select_area(req, res) {
  ...
  let { pin } = req.body;
  connection.query(
    "SELECT * FROM `vendor_service_area` where pin = " + pin + "",
    (err, rows, fields) => {
      if (err) {
        console.log(err);
        ...
        if (rows.length) {
          rows.forEach((item, index) => {
            ...
            if (vendors_id_array.includes(`${req.vendor_id}`)) {
              if (index === pin_area_length - 1) {
                res
                  .status(200)
                  .json({ status: true, response: "already selected" });
              }
            } else {
              let new_str = vendors_id + req.vendor_id + ",";
              //=====================================================================
              connection.query(
                "UPDATE `vendor_service_area` SET `vendors_id` ='" +
                  new_str +
                  "' where area_id = " +
                  area_id +
                  "",
                (err, rows, fields) => {
                  if (err) {
                    ...
                    if (index === pin_area_length - 1) {
                      res
                        .status(200)
                        .json({ status: false, response: "find some error" });
                    }
                  } else {
                    ...
                    if (index === pin_area_length - 1) {
                      res.status(200).json({
                        status: true,
                        response: "succefull added your sevice area",
                        your_service_area_list: [respo_array[0]],
                      });
                    }
                  }
                }
          ...
          res
            .status(200)
            .send({ status: false, response: "area_id not matched" });
        }
      }
    }
  );
...
export function vendor_service_area_list(req, res) {
  if (req.vendor_id) {
    let query_ =
      "SELECT * FROM vendor_service_area WHERE FIND_IN_SET('" +
      req.vendor_id +
      "', vendors_id) ";
      ...
      if (err) {
        ...
        if (rows.length) {
          rows.forEach((item, index) => {
            ...
            var presentValues = vendor_id.filter((value) =>
              vendors_id_array.includes(value)
            );
            var absentValues = vendor_id.filter(
              (value) => !vendors_id_array.includes(value)
            );
            ...
            if (index === pin_area_length - 1) {
              ...
              res.status(200).json({
                status: status_,
                service_available: presentValues,
                service_not_available: absentValues,
              });
            }
          });
        } else {
