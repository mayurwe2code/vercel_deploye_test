...
import payment_route from "./src/routers/paymentRouter.js";
...
app.get("/version", (req, res) => {
  ...
  res.send({
    latest_update: dat,
    latest_commit: "check port- 9999  -  server-good 25/11/2023",
    data: "author- we2code Pvt. Ltd."
  });
});

try {
  mongoose.connect(
    "mongodb+srv://Raahul_verma:vw48MlF9mMcMJL7y@cluster0.hxtq31y.mongodb.net/CrudNew?retryWrites=true&w=majority",
    {
    ...
    async (err, rows, fields) => {
      if (err) {
        ...
        if (err.code == "ER_DUP_ENTRY") {
          res
            .status(200)
            .send({ status: "200", response: "email already exist" });
        } else {
